    ...
    if (ids.length > 0) {
      ...
      profiles?.forEach((p) => {
        ...
        if (conv) {
          ...
          conv.other_role = p.role === 'admin' ? 'Soporte' : p.role === 'nurse' ? 'Enfermero/a' : 'Usuario';
        }
      });
    }
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/(client)/chat/${item.other_id}` as any)}
            >
              <View style={styles.avatarWrap}>
                {item.other_avatar ? (
                  <Image source={{ uri: item.other_avatar }} style={styles.avatar} />
                ) : (
                  <View style={[styles.avatarFallback, item.other_role === 'Soporte' && styles.supportFallback]}>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Tus mensajes con enfermeros aparecerán aquí.</Text>
          </View>
        }
      />
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  supportFallback: { backgroundColor: '#FEF3C7' },
