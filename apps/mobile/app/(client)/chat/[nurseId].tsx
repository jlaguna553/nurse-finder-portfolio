...
export default function ClientChat() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('*')
      .or(
        `and(sender_id.eq.${profile.id},receiver_id.eq.${nurseId}),` +
        `and(sender_id.eq.${nurseId},receiver_id.eq.${profile.id})`
      )
  ...
  useEffect(() => {
    supabase.from('profiles').select('full_name, avatar_url, role').eq('id', nurseId).single()
      .then(({ data }) => {
        if (data) {
          setOtherName(data.full_name ?? 'Usuario');
          setOtherAvatar(data.avatar_url);
          setOtherRole(data.role === 'admin' ? 'Soporte' : data.role === 'nurse' ? 'Enfermero/a' : '');
        }
      });
    ...
    const channel = supabase
      .channel(`chat_client_${profile?.id}_${nurseId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
        const msg = payload.new as Message;
        if (
          (msg.sender_id === profile?.id && msg.receiver_id === nurseId) ||
          (msg.sender_id === nurseId && msg.receiver_id === profile?.id)
        ) {
  ...
  }, [nurseId, profile?.id, fetchMessages]);
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          {otherAvatar ? (
            ...
            <View style={[styles.headerAvatarFallback, otherRole === 'Soporte' && styles.supportAvatar]}>
              <Text style={styles.headerAvatarText}>{initials}</Text>
            </View>
          )}
          <View>
            <Text style={styles.headerName}>{otherName}</Text>
            {otherRole ? <Text style={styles.headerSub}>{otherRole}</Text> : null}
          </View>
        </View>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyText}>Inicia la conversación con {otherName}</Text>
            </View>
          }
        />
...
const styles = StyleSheet.create({
  ...
  supportAvatar: { backgroundColor: '#FEF3C7' },
